...
from chemvae.metrics import compute_plogp, filter_valid
from chemvae.smiles_vae import SmilesVAE
from chemvae.smiles_vocab import SmilesVocabulary
...
def obj_func_gmm(z, vae, gmm: GMM, smiles_vocab: SmilesVocabulary):
    z_tensor = z
    z = z.to(torch.float32)
    for _ in range(100):
        smiles_list = vae.generate(z, deterministic=False)
        ...
        if success_list:
            smiles_list = success_list[:1]
            break
    plogp_tensor = gmm_predict(gmm, z_tensor)[:][0]

    return plogp_tensor, smiles_list


def gen_gmm(
    z_tensor: torch.Tensor, plogp_tensor: torch.Tensor, n_components: int
) -> GMM:
    y = plogp_tensor.view(-1, 1)
    X = torch.cat((z_tensor, y), dim=1)

    bgmm = BayesianGaussianMixture(n_components=n_components, covariance_type="diag")
    bgmm.fit(X.numpy())
    gmm = GMM(
        n_components=len(bgmm.weights_),
        priors=bgmm.weights_,
        means=bgmm.means_,
        covariances=np.array([np.diag(c) for c in bgmm.covariances_]),
    )
    gmm.from_samples(X.numpy())

    return gmm


def gmm_predict(gmm: GMM, z_tensor: torch.Tensor) -> torch.Tensor:
    z_tensor_index = list(range(0, z_tensor.size(dim=1)))
    y_pred = gmm.predict(z_tensor_index, z_tensor.numpy())
...
if __name__ == "__main__":
    ...
    gmm = gen_gmm(z_tensor, plogp_tensor, latent_dim)
    n_trial = 500
    ...
    for each_trial in range(n_trial):
        ...
        candidate, acq_value = optimize_acqf(
            UCB,
            bounds=torch.stack(
                [-0.1 * torch.ones(latent_dim), 1.1 * torch.ones(latent_dim)]
            ),  # 書籍版
            # bounds=torch.stack([torch.zeros(latent_dim), torch.ones(latent_dim)]),  # GitHub版
            q=1,
            num_restarts=5,
            raw_samples=10,
        )
        ...
        plogp_val, each_smiles_list = obj_func_gmm(
            unnormalized_candidate, vae, gmm, smiles_vocab
        )
